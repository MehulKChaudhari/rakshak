...
def cv_tf_train_test(df_done, label, v, ngram):
    ...
    f1_score_data = {'F1 Score': [f1_score(lr.predict(xtest_cv1), ytest), f1_score(knn.predict(xtest_cv1), ytest),
                                  f1_score(bnb.predict(xtest_cv1), ytest), f1_score(mnb.predict(xtest_cv1), ytest),
